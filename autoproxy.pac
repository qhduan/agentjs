 ...
 * Generated: Sun, 15 Jan 2017 09:24:00 +0000
 * GFWList Last-Modified: Fri, 13 Jan 2017 07:49:03 +0100
...
function FindProxyForURL(url, host) {
    for (var i = 0; i < rules.length; i++) {
        ret = testHost(host, i, rules, proxy);
        if (ret != undefined)
            return ret;
    }
    /*
        for (var i = 0; i < rulesCN.length; i++) {
            ret = testHost(host, i, rulesCN, proxyCN);
            if (ret != undefined)
                return ret;
        }
    */
...
function testHost(host, index, rules, proxy) {
